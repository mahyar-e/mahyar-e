## Hi there 👋



# Mohammad Mahyar Esfahani

## About Me
I am a physics researcher with a focus on **cosmology and gravitational waves**, currently working on the **stochastic gravitational wave background (SGWB)**. My academic interests lie at the intersection of **theoretical physics, data analysis, and scientific computing**. In parallel with research, I have teaching experience in undergraduate physics and enjoy translating complex physical concepts into clear, structured explanations.

## Research Interests
- Stochastic gravitational wave background (SGWB)
- Cosmology and early-universe physics
- Statistical field theory
- Gravitational wave data analysis
- Large-scale structure and cosmological perturbation theory

## Academic & Teaching Experience
- Teaching Assistant — *Elementary Physics I* (Undergraduate level)
- Experience in problem-solving sessions, grading, and concept-driven instruction
- Strong interest in pedagogical clarity and physics education

## Technical Skills
**Programming & Scientific Computing**
- Python (NumPy, SciPy, Matplotlib, Jupyter)
- LaTeX (academic writing, lecture notes, reports)
- Markdown for technical documentation

**Data Analysis**
- Signal processing and statistical inference
- Visualization of high-dimensional scientific data
- Reproducible research workflows

## Selected Topics & Repositories
This GitHub profile includes repositories related to:
- Cosmology exercises and derivations
- Gravitational wave data analysis tools
- Machine learning applications in physics (CNNs, LSTMs)
- Lecture notes and educational material
- Numerical simulations and visualization scripts

## Philosophy
I value:
- **Clarity over cleverness**
- **Reproducibility in scientific research**
- **Strong theoretical foundations paired with practical computation**

## Contact
- Email: mahyar137979@gmail.com

---

> *“Physics is not about finding the right answers — it is about asking the right questions.”*




### 🛠️ Technologies & Tools
![Python](https://img.shields.io/badge/-Python-3776AB?style=flat-square&logo=python&logoColor=white)


<!--
**mahyar-e/mahyar-e** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.

Here are some ideas to get you started:

- 🔭 I’m currently working on ...
- 🌱 I’m currently learning ...
- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 💬 Ask me about ...
- 📫 How to reach me: ...
- 😄 Pronouns: ...
- ⚡ Fun fact: ...
-->
